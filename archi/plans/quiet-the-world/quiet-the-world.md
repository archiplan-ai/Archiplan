...
Answer the debt the world's first hour of real use produced. Coverage stops asking a question
...
siblings, the closing refusal needs a world to refuse against, and the briefing stops
...
- `DocsCompiler` — holds the world: what coverage asks, what `.worldignore` answers, and the one scenario digest both the plan and the link read
...
- `Planner` — the closing refusal fires only where a world stands
