...
the empty-block refusal needs a wing to refuse against
...
- the refusal asks whether the tree holds a wing at all before it fires
...
- test — plan_e2e: a post-wing plan on a tree with no world facts at all closes without a refusal
- test — plan_e2e: a post-wing plan on a tree that has a wing refuses until a fact covers one of its nodes
