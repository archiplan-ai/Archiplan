...
wing. A cycle in `uses` is a located error naming the ring. `world_chain_deep` reports a
`uses` chain that runs deeper than three, because the wing then holds a theory of the
...
into the other wing's vocabulary.
