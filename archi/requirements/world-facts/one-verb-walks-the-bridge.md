...
condition one element, which is the question the world exists to answer. `--json` emits
the structured envelope. The listing reads the tree and resolves nothing against a pin.

## System Context

`the-world-is-reached-by-traversal` traded findability by phrase for simplicity, on the
...
an operator walks by opening files, and the world gets reached the one way the decision
...
- test — a tree with no world prints nothing and exits zero
