...
indistinguishable from an observed one by every mechanism the wing has.

## Attractor

The wing fills with facts that read like observations and were written by a language
...
the tree does not know whether there was one. The wing ends up the most authoritative
...
`the-wing-checks-form-and-never-truth` already settled: no command decides truth, and the
