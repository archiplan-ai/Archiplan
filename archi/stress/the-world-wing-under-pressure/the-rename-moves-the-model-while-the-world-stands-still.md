...
condition is word for word what it was. The wing built to outlive the design breaks
...
requirement that names the moved path. The wing inherits the contract instead of
inventing a second one, so the pressure finds no new failure here. What it does find
belongs to the model-wide question of renames, not to this wing.
