...
# Two branches edit one world

Two seats work in parallel and both touch the world: one edits a fact's scenarios, the
...
`remint` and folds concurrent stress rounds with `session fold`. The world has no
...
without a conflict. The world has no shared file: one fact is one file, so a same-slug
