...
# The standing project has no world

Upgrade a project that has run for a year into a binary that knows the world. There is
...
Either the world arrives loud — a finding per unconditioned node on a tree that changed
...
without it. The count that `the-check-counts-the-world` prints reads zero on a tree
where the world is absent and on a tree where it was emptied, and those are not the
same state.

## Resolution

The world arrives silent and announced: derived `the-world-arrives-without-noise`. No
