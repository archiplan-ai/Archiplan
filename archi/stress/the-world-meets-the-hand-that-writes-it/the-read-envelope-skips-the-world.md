...
# The read envelope skips the world
...
gives an agent the conditions bearing on the work it was handed. The world was built so an
agent writing code knows why the behavior is what it is, and the agent's own read surface
does not carry it.

## Attractor

Agents keep working the way they did before the world existed. They read requirements,
...
The read surface carries the world: derived `the-read-envelope-carries-the-conditions`, and
...
reader who did not know to ask, which is the reader the world was built for.
