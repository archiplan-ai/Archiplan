...
The statement layer is built for agents and compilers: failures are machine-readable, batches are atomic, and
identical retries are always safe.
...
- **noop** — the statement restates something identical that already exists (a `define` of an element already
  defined that way, or restating an existing edge). Replays and at-least-once retries are safe, so recompiling
  unchanged source re-applies the whole batch as no-ops.
...
| code | raised when |
|------|-------------|
| E_PARSE | the statement is not a well-formed statement object: unknown `stmt` (including the retired mutation verbs), missing or ill-typed field, malformed path; the offending field is named |
| E_UNKNOWN_NAME | a referenced node / type / view / path does not resolve; `kind` says which |
| E_REDECLARED | a `define` differs from the existing definition of the name — including a rel / conn kind mismatch, or a divergent restatement of a stdlib name; the existing definition is included |
...
| E_STDLIB_PROTECTED | attempt to tag a stdlib (preset) edge into views — tags on it would not survive a dump replay |
...
Errors reject statements. States that are legal mid-construction but suspect — carried traffic matching no
delegation, a declared port with no attachments at all (`unused_port`), a view with no edges, a type with no
instances — are **findings**: surfaced by `check`/`query` ([queries](./queries.md)), never by rejecting a
statement. Findings describe a model the source compiled cleanly; they are advisory.
