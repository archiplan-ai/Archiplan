...
1. **Merge.** Conflicts, if any, land in content files (resolve normally)
   ...
   `archi/links/.gitattributes`. A seat landing that stopped on conflicts
   (`archi worktree merge` — the archi-finish-worktree skill) arrives here
   mid-merge: same triage, and after the join its re-run finishes the
   retire.
...
**Where it runs: in the later round's seat, never in the primary
checkout** — under the seat discipline the primary refuses mutation, and
that refusal is correct: the repair is the later writer's work. If that
seat was already retired (`--to` landed it), re-attach it first —
`archi worktree mint <slug>` picks the surviving branch back up — then
merge the receiving branch *into* the seat, run the ceremony there, and
land again. A stale `--to` branch from the failed landing must be deleted
(or a fresh name chosen) before the re-landing.
